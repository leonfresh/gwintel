...
export const calculateStats = (performances: WarPerformance[]): MemberStats[] => {
  ...
  const normalizeKey = (name: string) => normalizeDisplayName(name).toLowerCase();
  ...
  const stats: MemberStats[] = Array.from(memberMap.entries()).map(([, data]) => {
  ...
  return stats.map((s) => {
    ...
    if (s.win_rate >= 95 && total_attacks >= 10) titles.push("Flawless Striker");
    if (s.win_rate >= 90 && total_attacks >= 15) titles.push("Ace Attacker");
    if (s.win_rate >= 80 && total_attacks >= 20) titles.push("Consistent Attacker");
    if (s.total_wins >= 25) titles.push("War Machine");
    if (s.total_wins >= 15 && s.total_losses <= 5) titles.push("Clean Finisher");
    if (total_attacks >= 30) titles.push("High Volume Attacker");
    if (s.total_wins >= 10 && s.total_wins >= s.total_losses * 2) titles.push("Dominant Offense");
    ...
    if (s.win_rate >= 65 && s.avg_def_wins >= 2) titles.push("Balanced Defender");
    if (s.win_rate >= 80 && s.avg_def_wins >= 3) titles.push("All-Rounder Elite");
